...
void ShaderRecord::ApplyDynamics()
{
...
int ShaderManager::AddShader(char *Filename, bool AllowDuplicates, UINT32 refID)
{
	...
	TextureManager	*TexMan=TextureManager::GetSingleton();
	NewShader->Effect->SetTexture("reflection",passTexture[OBGEPASS_REFLECTION]);
	...
	NewShader->Effect->SetBool("bHasReflection",!!passTexture[OBGEPASS_REFLECTION]);
...
void ShaderManager::OnResetDevice()
{
	...
	while(SShader!=StaticShaders.end())
	{
		...
		(*SShader)->Effect->SetTexture("reflection",passTexture[OBGEPASS_REFLECTION]);
		...
		(*SShader)->Effect->SetBool("bHasReflection",!!passTexture[OBGEPASS_REFLECTION]);
	...
	while(Shader!=Shaders.end())
	{
		...
		Shader->second->Effect->SetTexture("reflection",passTexture[OBGEPASS_REFLECTION]);
		...
		Shader->second->Effect->SetBool("bHasReflection",!!passTexture[OBGEPASS_REFLECTION]);
